...
- `GET /api/landlord/properties` and the `COMPLETED` status on
  `PATCH /api/landlord/requests/:id` were added to the backend during this
  project (originally missing — no way for a landlord to list their own
  listings, and no way for a rental to ever reach `COMPLETED`, which reviews
  require) and are now live.
- Payments use SSLCommerz. Checkout opens in a new tab from `.../pay`; the
  original tab polls `GET /api/payments/:id` for the status change instead of
  relying on SSLCommerz's redirect, which posts straight to the backend
